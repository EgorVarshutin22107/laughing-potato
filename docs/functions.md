...
```mermaid
graph TD
    A[Пользователь] -->|Запустить игру| B[Главное меню]
    B -->|Генерация лабиринта| C[Лабиринт с монетами и персонажем]
    B -->|Выйти из игры| Z[Приложение закрывается]

    C -->|Использовать клавиши управления| D[Перемещение по лабиринту]
    D -->|Нажать стрелочную клавишу| E[Персонаж перемещается]

    C -->|Переместиться к монетке| F[Сбор монет]
    F -->|Пройти через монетку| G[Увеличить счетчик монет]
    F -->|Пройти через замедляющую монетку| H[Замедление на 5 секунд]

    C -->|Истекло время: 120 секунд| I[Завершение игры]
    I -->|Вывести статистику| J[Конец игры]

    C -->|Нажать кнопку ё| K[Подтверждение выхода]
    K -->|Нажать Выйти из игры| L[Вернуться в главное меню]
    K -->|Нажать Отмена| C

    L -->|Выход из игры| Z[Приложение закрывается]	
```
